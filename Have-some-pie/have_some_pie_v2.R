...
# Set up custom linters with longer line length
# install.packages("psych")
# BiocManager::install("RCy3")
# install.packages("dendroNetwork")
# my_linters = lintr::linters_with_defaults( line_length_linter = lintr::line_length_linter(length = 150L) )
# lintr::lint("network_igraph.R", linters = my_linters)
...
fetching_files = function(){
    ...
    taxa_file = "/Users/danielcm/Desktop/diammatics/T1D/Phyloseq/ps_s2_final.csv"
    maaslin_filt_file = "/Users/danielcm/Desktop/diammatics/T1D/Maaslin2.2/w9w10/Pathway/maaslin2_Week_and_consortia_pathway_NS1_w9w10_vs_S2_w9w10_ref_Week_and_consortia,S2_w9w10/all_results.tsv"
    metadata_file = "/Users/danielcm/Desktop/diammatics/T1D/metadata_ps.csv"
    output_path = "/Users/danielcm/Desktop/diammatics/T1D/Maaslin2.2/Networks/S2"
    filtering_category = "Week_and_consortia"
    filtering_value = "S2_w9w10"
    alpha_sign = 0.05
    rho_minimum = 0.7
...
normalizing_and_filtering = function(pathway_file, taxa_file, maaslin_filt_file, metadata_file, output_path, filtering_category, filtering_value, alpha_sign, rho_minimum, ref_value) {
    # Open and read files
    ...
    filtered_IDs = metadata_df[metadata_df[[filtering_category]] == filtering_value, ] # Filtering metadata to only include samples with the desired value in the specified category
    pathway_mat = pathway_mat[, colnames(pathway_mat) %in% rownames(filtered_IDs)] #Filtering pathway matrix to only have the samples present in the filtered metadata
    taxa_mat = taxa_mat[, colnames(taxa_mat) %in% rownames(filtered_IDs)] #Filtering taxa matrix to only have the samples present in the filtered metadata
    ...
    # Normalize by CLR and add pseudocount of 0.000001 to avoid issues with zeros
    pathway_mat_t = t(pathway_mat) # Transpose to have samples as rows and pathways as columns for clr function
    taxa_mat_t = t(taxa_mat) # Transpose to have samples as rows and taxa as columns for clr function

    clr_pathway = as.matrix(clr(pathway_mat_t + 1)) # Adding a pseudocount of 1 to avoid issues with zeros
    clr_taxa = as.matrix(clr(taxa_mat_t + 1)) # Adding a pseudocount of 1 to avoid issues with zeros
    ...
    print(colnames(clr_pathway))
    print(rownames(clr_pathway))
    ...
    common_samples = intersect(colnames(clr_pathway), colnames(clr_taxa)) # This wll print the common samples between the two matrices
    unique_samples = setdiff(colnames(clr_pathway), colnames(clr_taxa)) # This will print the samples that are unique to clr_pathway, in theory only week 7 should be different
    ...
    for (i in 1:nrow(clr_taxa)) {
        for (j in 1:nrow(clr_pathway)) {
            # print(paste0("Processing taxa ", i, " of ", nrow(clr_taxa), " and pathway ", j, " of ", nrow(clr_pathway)))
    ...
    sign_ass = sum(as.numeric(clean_correlation_test > rho_minimum & pval_matrix_clean < alpha_sign)) # This will give you the number of significant correlations with rho > 0.8 and adjusted p-value < 0.001
    ...
    writeLines(c(sentence1, sentence2, sentence3, sentence4, sentence5, sentence6, sentence7, sentence8, sentence9), paste0(output_path, "/summary_analysis.txt"))
    write.table(clean_correlation_test, file = paste0(output_path, "/spearman_clean_correlation_test.tsv"), sep = "\t", quote = FALSE, col.names = NA) # This saves a file where the columns/rows with NAs are removed from the Spearman correlation in taxa and pathways
    ...
    write.table(enriched_clean_correlation_test, file = paste0(output_path, "/spearman_maaslin2_enriched_clean_correlation_test.tsv"), sep = "\t", quote = FALSE, col.names = NA) # This one writes a file with the spearman correlation only found in the pathways enriched in maaslin2 when comparing it against another group.
    beepr::beep(6) # Beep when done
    list_of_matrices = list(correlation_test, clean_correlation_test, pval_matrix_clean, significant_matrix, enriched_clean_correlation_test)
    return(list_of_matrices)
}

heatmap_plotting = function(matrix_to_plot, output_path) {
    png(file.path(output_path, "maaslin2_significant_correlation_heatmap_plot.png"), width = 2400, height = 2400, res = 150)
    ...
    write.table(colnames(matrix_to_plot), file = paste0(output_path, "pathway_names_same_order_as_heatmap.tsv"), sep = "\t", quote = FALSE, col.names = FALSE)
    dev.off()
}
...
make_network = function(pathway_file_name, taxa_file_name, maaslin_file_name, metadata_file_name, output_path, rho_minimum, filtering_category, filtering_value,
                        community, timepoint, clean_correlation_matrix_file, ref_value) {
    ...
    path_matrix_final = path_matrix_clean[rowSums(path_matrix_clean) > 0, , drop = FALSE] #49 rows are pathways, 52 columns are mice
    ...
    log_file = file.path(output_path, "/network_summary_analysis.txt")
    ...
    degree_taxa = pivoted_edge_table %>% count(taxon, wt = (filtered_rho != 0), name = "degree") #Centrality degree calculation in taxa
    degree_pathway = pivoted_edge_table %>% count(pathway, wt = (filtered_rho != 0), name = "degree") #Centrality degree calculation in pathways
    ...
    nodes_table = tibble(id = nodes_ids_in_both) %>%
                                mutate(
                                    group = case_when(
                                        id %in% gsub("_", " ", rownames(taxa_matrix_final)) ~ "taxa",
                                        id %in% gsub("_", " ", rownames(path_matrix_final)) ~ "pathway",
                                        TRUE ~ "unknown"
                                    ),
                                    score = case_when(
                                        group == "taxa" ~ {
                                ...
                                mutate(degree = ifelse(is.na(degree.x), degree.y, degree.x))
    
    nodes_table = nodes_table %>% mutate(id = gsub("_", " ", id)) #Replacing underscores with spaces for better visualization in Cytoscape

    #This block was working when I was doing normalization of pathways by samples (CONCEPTUALLY WRONG)
    #nodes_table = bind_rows(tibble(id = rownames(taxa_matrix_final), #Typically I would get a size incompatibility error, but because the names of my vectors are the same as the node names, it matches them properly
    #                                group = "taxa", score = as.numeric(num_of_mice_in_taxa)),
    #                        tibble(id = rownames(path_matrix_final), group = "pathway", score = as.numeric(num_of_mice_in_pathway))) %>%
    #                                left_join(degree_taxa, by = c("id" = "taxon")) %>%
    #                                left_join(degree_pathway, by = c("id" = "pathway")) %>%
    #                                mutate(degree = ifelse(is.na(degree.x), degree.y, degree.x))
    ...
    createNetworkFromDataFrames(nodes_table, pivoted_edge_table,
                                title = paste0("Taxa-Pathway Network for ", community, " at ", timepoint, " (rho >= ", rho_minimum, ")"),
                                collection = "Diammatics_T1D_Networks")
    ...
    setNodeLabelColorDefault(new.color = "white",                           #Setting default font color to white
                            style.name = "default")
    if(min(nodes_table$score) == max(nodes_table$score)){
        opacity_value = c(min(nodes_table$score)-1, max(nodes_table$score)) #If all nodes have the same score, we need to make one different to avoid errors in mapping
    }
    else {
        opacity_value = c(25, (round(25+2.3*(max(nodes_table$score))))) #Scaling opacity from 25 to a max of proportional part of mice
    }
    
    setNodeFillOpacityMapping(table.column = "score", #The score (number of mice having that taxa/pathway) defines the opacity of the node
                        table.column.values = c(1,100),
                        opacities = opacity_value, #Scaling opacity from 25 to a max of proportional part of mice
    ...
    if(longnames == TRUE){
       setNodeLabelPositionDefault(new.nodeAnchor = "S",
                               ...
                               new.justification = "c",
                               new.xOffset = 0, 
                               new.yOffset = 15,  # Move further down
                               style.name = "default")
    } else {
    ...
    setNodeSizeMapping(table.column = "degree", #The degree of the node defines its size, in this case, the number of connections in a node
                    table.column.values = c(min(nodes_table$degree), max(nodes_table$degree)),
                    sizes = c(20, 100),
                    mapping.type = "continuous",
                    style.name = "default")
...
global = function() {
    ...
    users_parameters = fetching_files()
    print(users_parameters[12])

    matrix_list = normalizing_and_filtering(
        pathway_file = users_parameters[1],
        taxa_file = users_parameters[2],
        maaslin_filt_file = users_parameters[3],
        metadata_file = users_parameters[4],
        output_path = users_parameters[5],
        filtering_category = users_parameters[6],
        filtering_value = users_parameters[7],
        alpha_sign = as.numeric(users_parameters[8]),
        rho_minimum = as.numeric(users_parameters[9]),
        ref_value = users_parameters[12])

    heatmap_plotting(matrix_to_plot = matrix_list[[5]], output_path = users_parameters[5])

    make_network(
        pathway_file_name = users_parameters[1],
        taxa_file_name = users_parameters[2],
        maaslin_file_name = users_parameters[3],
        metadata_file_name = users_parameters[4],
        output_path = users_parameters[5],
        filtering_category = users_parameters[6],
        filtering_value = users_parameters[7],
        rho_minimum = as.numeric(users_parameters[9]),
        community = users_parameters[10],
        timepoint = users_parameters[11],
        clean_correlation_matrix_file = matrix_list[[5]],
        ref_value = users_parameters[12])
}

global()

#Taxa matrix must have samples in columns and taxa (species names!!!) in rows

#/Users/danielcm/Desktop/diammatics/T1D/PICRUSt2.2/Pathway_merged_metagenome.tsv
#/Users/danielcm/Desktop/diammatics/T1D/Phyloseq/ps_ns1_final.csv
#/Users/danielcm/Desktop/diammatics/T1D/Maaslin2.2/same_community/Pathway/maaslin2_Week_and_consortia_pathway_NS1_w5_vs_NS1_w9w10_ref_Week_and_consortia,NS1_w9w10/all_results.tsv
#/Users/danielcm/Desktop/diammatics/T1D/metadata_ps.csv
#/Users/danielcm/Desktop/diammatics/T1D/PICRUSt2.2/ns1_w5_network_output