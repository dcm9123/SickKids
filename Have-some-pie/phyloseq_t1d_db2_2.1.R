...
subsetting_weeks_and_consortia = function(ps_weeks){
  ...
  for (j in seq_along(consortia_names)) {
    ...
    not_week_7 = sample_data(ps_by_consortia[[cons_label]])$Timepoint != "week_7"
    ps_by_consortia[[cons_label]] = prune_samples(not_week_7, ps_by_consortia[[cons_label]]) #Removing week 7 from consortia as it was not in the experimental design}
  }
...
#Returns: Nothing
finding_overlap = function(ps_clustered_all_weeks_together,sample_type){
  duplicates = list()
  combined=list()
  if(sample_type=="weeks"){
    for(i in seq_along(ps_clustered_all_weeks_together)){
      item = ps_clustered_all_weeks_together[[i]]
